...
contract Remittance is Stoppable {
    ...
    function hashOTP(address exchange, bytes32 password) private view returns(bytes32 hash) {
    ...
    function cancelTransaction(address exchange, bytes32 password) public returns(bool success) {
        require(exchange != address(0) && password != 0, "Both exchange and password must be set");
        
        bytes32 hash = hashOTP(exchange, password);
        Transaction memory tx = transactions[hash];
